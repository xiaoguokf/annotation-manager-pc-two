    ...
    <el-card class="flex-1 flex flex-col">
      ...
      <div class="search-area">
        <el-form :model="searchForm" class="search-form" :inline="true">
          ...
          <el-form-item>
            <el-switch v-model="onlyReturned" active-text="仅回传" @change="handleOnlyReturnedChange" />
          </el-form-item>
          <el-form-item>
      ...
      <div class="flex-1 min-h-0 overflow-hidden">
        <el-table :data="tableData" v-loading="loading" height="100%" width="100%" row-key="searchId"
          ...
          <el-table-column prop="status" label="审核状态" width="140">
            <template #default="{ row }">
              <el-tag v-if="getStatusColorConfig(row.status).color" :color="getStatusColorConfig(row.status).color"
          ...
          <el-table-column label="操作" width="340" fixed="right">
            <template #default="{ row }">
              ...
              <el-button type="primary" size="small" link :disabled="!canPreviewAudit(row.status)" @click="handlePreviewAudit(row as ProjectReviewListVO)">
                <el-icon>
                  <Icon icon="ep:view" />
                </el-icon>
                审核预览
              </el-button>
              <el-button type="primary" size="small" link @click="handleView(row as ProjectReviewListVO)">
                <el-icon>
...
import { postProjectReviewClaimApi, postProjectReviewClaimSearchIdApi, getProjectReviewListApi, type ProjectReviewQuery, type ProjectReviewListVO } from '@/api/gen/projectController'
...
const isMaterialReviewStatus = (status: number | undefined): boolean => {
  return status === ProjectStatus.MATERIAL_PENDING_REVIEW || status === ProjectStatus.MATERIAL_PROBLEM_SUBMITTED || status === ProjectStatus.ADMIN_MATERIAL_REJECTED
}
...
const canPreviewAudit = (status: number | undefined): boolean => {
  return status === ProjectStatus.REVIEW_SUCCESS || status === ProjectStatus.PROBLEM_SUBMITTED || status === ProjectStatus.MATERIAL_PROBLEM_SUBMITTED
}
