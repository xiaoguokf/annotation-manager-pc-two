...
const handleLogin = async () => {
  ...
  try {
    ...
    if (data.code === 200) {
      ...
      console.log('登录响应数据:', { token, refreshToken, user })
      ...
      if (refreshToken) {
        ...
        console.log('RefreshToken已存储:', refreshToken)
      }
      ...
      if (user) {
        const userInfo = {
          ...
          roles: user.roles || [],
          deptIds: [], // 根据实际接口返回的数据调整
          avatar: '' // 根据实际接口返回的数据调整
        }
      ...
      console.log('Token验证:', getToken())
      
      // 检查是否有redirect参数
      const redirect = route.query.redirect as string
      if (redirect) {
        console.log('跳转到redirect页面:', redirect)
        ...
        <div class="mb-10">
          <div class="flex items-center space-x-4 mb-6">
            <div class="w-16 h-16 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30">
              <img src="/logo-shortclip.svg" alt="Logo" class="w-10 h-10" />
            </div>
